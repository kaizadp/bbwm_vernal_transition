...
gg_NH4 = 
 ...
 # scale_color_manual(values = c("#1d457f", "#f57946"))+
   scale_shape_manual(values = c(1,16))+
  
   labs(
 #   title = "A",
    x = "",
    y = expression(bold("NH"[4]^+{}*"-N, mg"^-1*" kg"^-1)))+
...
#    title = "B",
    x = "",
    y = expression(bold("NO"[3]^-{}*"-N, mg"^-1*" kg"^-1)))+
...
#library(cowplot)
#plot_grid(gg_NH4, gg_NO3, nrow = 2, labels = c("A","B"))
...
gg_vt(stream_daily, aes(x = as.Date(Date), y = gageht_cm))+
  geom_path()+
  geom_point(data = stream_n, aes(x = as.Date(Date), y  = NO3_N_mg_L*30 + 145, shape=Watershed), 
             color = "dodgerblue3", size=2.5)+
  scale_y_continuous(sec.axis = sec_axis(~(.-145)/30, 
                                         name = expression(bold("NO"[3]^-{}*"-N, mg"^-1*" L"^-1))), 
                     limits = c(142.5,175))+
  scale_shape_manual(values = c(1,16))+
  labs(
#    title = "B",
